...
const sandwichUniswapV2RouterTx = async (txHash) => {
  ...
  const userMinRecv = await getUniv2ExactWethTokenMinRecv(BigInt(amountOutMin), path);
  ...
  // Extract gas
  const frontsliceGas = BigInt(simulatedResp.results[0].gasUsed);
  const backsliceGas = BigInt(simulatedResp.results[2].gasUsed);

  // Bribe 99.99% :P
  const bribeAmount = sandwichStates.revenue - frontsliceGas * nextBaseFee;
  const maxPriorityFeePerGas = (bribeAmount * 9999n) / 10000n / backsliceGas;
  ...
  if (maxPriorityFeePerGas < nextBaseFee) {
    logTrace(
      strLogPrefix,
...
const main = async () => {
  ...
  void wssProvider.on(
    "pending",
    (txHash) =>
      void sandwichUniswapV2RouterTx(txHash).catch((e) => {
