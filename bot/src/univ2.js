...
export const getUniv2DataGivenIn = (aIn, reserveA, reserveB) => {
  ...
  const numerator = aInWithFee * reserveB;
  const denominator = aInWithFee + reserveA * 1000n;
  const bOut = numerator / denominator;

  // Underflow
  let newReserveB = reserveB - bOut;
  if (newReserveB < 0n || newReserveB > reserveB) {
    newReserveB = 1n;
  }
...
export const getUniv2DataGivenOut = (bOut, reserveA, reserveB) => {
  ...
  const numerator = reserveA * bOut * 1000n;
  const denominator = newReserveB * 997n;
  const aAmountIn = numerator / denominator + 1n;
